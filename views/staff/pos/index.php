  ...
  <main class="pos-workspace flex-1 flex overflow-hidden">
    ...
    <div class="flex items-center justify-between mb-5">
      <div>
        ...
        <p class="mt-1 text-xs text-gray-500 dark:text-white/40">Selling as <span class="font-semibold text-ink dark:text-white"><?= htmlspecialchars($profile['branch_name']) ?></span> — every sale here is tagged to this branch.</p>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-ink-2 border border-gray-100 dark:border-white/10 rounded-2xl p-4 mb-5 shadow-sm">
      ...
      <?php if (empty($products)): ?>
        <div class="bg-white dark:bg-ink-2 border border-gray-100 dark:border-white/10 rounded-2xl p-8 text-center shadow-sm">
          <p class="text-gray-500 dark:text-white/50 text-sm">No sellable products for this branch yet. Ask your seller/manager to assign products with stock to this branch.</p>
        </div>
      <?php else: ?>
...
function posApp_searchCustomers(query) {
  ...
  posSearchTimer = setTimeout(() => {
    fetch('/staff/pos/customers?q=' + encodeURIComponent(query))
      .then(res => res.json())
      .then(customers => {
  ...
  fetch('/staff/pos/checkout', {
    ...
    .then(result => {
      if (result.success) {
        ...
        posApp_renderCart();
        alert('Sale completed! Order #' + result.order_id);
      } else {
