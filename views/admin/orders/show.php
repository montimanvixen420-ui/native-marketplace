<div class="flex min-h-screen bg-gray-50">
  ...
  <main class="flex-1 px-8 py-8">
    <div class="max-w-2xl">
      <a href="/admin/orders" class="text-sm text-gray-400 hover:text-teal">&larr; Back to orders</a>
      ...
      <?php if (!empty($order['branch_id'])): ?>
        <div class="mt-6 bg-amber-50 border border-amber-200 rounded-lg p-5 text-sm text-amber-800">
          ...
          <p class="mt-1">This order was placed for <?= htmlspecialchars($order['branch_name'] ?? 'a specific branch') ?>. Only that branch's order staff can update its fulfillment status.</p>
        </div>
      <?php elseif (!empty($nextStatuses)): ?>
        <form method="POST" action="/admin/orders/<?= (int) $order['id'] ?>/fulfillment" class="mt-6 bg-white border border-gray-200 rounded-lg p-5">
