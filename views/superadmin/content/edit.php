<div class="flex min-h-screen bg-gray-50">
  ...
  <main class="flex-1 px-8 py-8 max-w-2xl">
    <div class="mb-6">
      ...
      <h1 class="font-display font-semibold text-2xl text-gray-900 mt-2">Edit content</h1>
      <p class="text-sm text-gray-500">Update this banner, announcement, or site text.</p>
    </div>
    ...
    <form method="POST" action="/superadmin/content/<?= (int) $item['id'] ?>/update" class="bg-white border border-gray-200 rounded-lg p-6 space-y-5">
      ...
      <div>
        ...
        <select name="type" class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2.5 focus:outline-none focus:border-teal">
          ...
          <option value="site_text" <?= $item['type'] === 'site_text' ? 'selected' : '' ?>>Site text</option>
        </select>
      </div>
      ...
      <div class="flex items-center gap-2">
        <input type="checkbox" name="is_active" id="is_active" <?= (int) $item['is_active'] === 1 ? 'checked' : '' ?> class="rounded border-gray-300 text-teal focus:ring-teal">
